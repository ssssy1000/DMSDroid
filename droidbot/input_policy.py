...
from .input_event import InputEvent, KeyEvent, IntentEvent, TouchEvent, SetTextEvent, KillAppEvent
from .utg import UTG
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def generate_event(self):
        ...
        print('Get current device state')
        ...
        if self.device.humanoid is not None:
            ...
            print('self.humanoid_view_trees---', self.humanoid_view_trees)
        event = None
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        self.logger.info(" %s" % current_state.state_str)
        ...
        print('Get all possible input events')
        if self.random_input:
