...
from .qlearning.explore import QExploration
from .ses import SES
...
class DroidBot(object):
    ...
    def __init__(self,
        ...
        if output_dir is not None:
            ...
            ses_index_path = pkg_resources.resource_filename("droidbot", "resources/ses.html")
            ...
            shutil.copy(ses_index_path, output_dir)
    ...
    def start(self):
        ...
        try:
            if self.timeout > 0:
                self.timer = Timer(self.timeout, self.start_timer2)
                self.timer.start()
            ...
            self.stop()
            sys.exit(-1)
        self.logger.info("exploration DroidBot Stopped")

    def start_timer2(self):
        if not self.enabled:
            return
        self.logger.info("Second Exploration")
        try:
            self.input_manager.stop()
            self.logger.info("First Exploration Stop")
            ses = SES(self.output_dir)
            # self.timer2 = Timer(50, self.stop)
            # self.timer2.start()
            if not self.enabled:
                return
            # 开始第二轮探索
            explorer = QExploration(self.device, self.app, self.device.output_dir)
            explorer.start()
            self.stop()
        ...
        self.stop()
        self.logger.info("start_timer2 DroidBot Stopped")

    def stop(self):
        self.enabled = False
        if self.timer and self.timer.is_alive():
            self.timer.cancel()
        if self.timer2 and self.timer2.is_alive():
            self.timer2.cancel()
        if self.env_manager:
