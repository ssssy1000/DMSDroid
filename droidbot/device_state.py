...
from .utils import md5, EDIT_TEXT
...
class DeviceState(object):
    ...
    def __generate_view_strs(self):
        for view_dict in self.views:
            ...
            self.__get_content_free_view_str(view_dict)
    ...
    def __get_view_signature(view_dict):
        ...
        signature = "[resource_id]%s[text]%s[content_description]%s[%s,%s,%s]" % \
    ...
    def __get_content_free_view_str(self, view_dict):
        if 'view_content_free_str' in view_dict:
            return view_dict['view_content_free_str']
        view_content_free_signature = DeviceState.__get_content_free_view_signature(view_dict)
        parent_strs = []
        for parent_id in self.get_all_ancestors(view_dict):
            parent_strs.append(DeviceState.__get_content_free_view_signature(self.views[parent_id]))
        parent_strs.reverse()
        child_strs = []
        for child_id in self.get_all_children(view_dict):
            child_strs.append(DeviceState.__get_content_free_view_signature(self.views[child_id]))
        child_strs.sort()
        view_content_free_str = "Activity:%s\nSelf:%s\nParents:%s\nChildren:%s" % \
                   (self.foreground_activity, view_content_free_signature, "//".join(parent_strs), "||".join(child_strs))
        import hashlib
        view_content_free_str = hashlib.md5(view_content_free_str.encode('utf-8')).hexdigest()
        view_dict['view_content_free_str'] = view_content_free_str
        return view_content_free_str
    ...
    def choose_input_text(self, view):
        for key in EDIT_TEXT:
            if key in view['signature']:
                return random.choice(EDIT_TEXT[key])
        return random.choice(EDIT_TEXT['default'])
    ...
    def get_possible_input(self):
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'editable'):
                text = self.choose_input_text(self.views[view_id])
