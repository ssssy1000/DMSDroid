...
class EventLog(object):
    ...
    def start(self, semantic_manager):
        """
        start sending event
        """
        ...
        # print('send state: ', self.from_state.__dict__)
        # print('send event: ', self.event)
        if isinstance(self.event, KillAppEvent) or isinstance(self.event, IntentEvent):
            print(f'cannot get {self.event_str} semantics')
        elif semantic_manager is None:
            print('Second Exploration dont need semantic events')
        else:
            semantic_manager.get_semantic_state(self.from_state, self.event)
...
class KeyEvent(InputEvent):
    ...
    def get_event_str(self, state):
        return "%s(state=%s, name=%s)" % (self.__class__.__name__, state.structure_str, self.name)
...
class UIEvent(InputEvent):
    ...
    def view_str(state, view):
        ...
        view_str = view['view_content_free_str'] if view_text.isdigit() else view['view_str']
        view_text = 'num' if view_text.isdigit() else view_text
        view_text = view_text[:8] if len(view_text) > 8 else view_text
        view_short_sig = f'{state.activity_short_name}/{view_class}-{view_text}'
        return f"state={state.structure_str}, view={view_str}({view_short_sig})"
...
class TouchEvent(UIEvent):
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(state=%s, x=%s, y=%s)" % (self.__class__.__name__, state.structure_str, self.x, self.y)
        else:
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(state=%s, x=%s, y=%s)" % \
                   (self.__class__.__name__, state.structure_str, self.x, self.y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
            raise InvalidEventException(msg)
...
class SwipeEvent(UIEvent):
    ...
    def get_event_str(self, state):
        if self.start_view is not None:
            ...
            start_view_str = "state=%s, start_x=%s, start_y=%s" % (state.structure_str, self.start_x, self.start_y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
            raise InvalidEventException(msg)
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(state=%s, x=%s, y=%s, direction=%s)" % \
                   (self.__class__.__name__, state.structure_str, self.x, self.y, self.direction)
        else:
            return "%s(state=%s, direction=%s)" % \
                   (self.__class__.__name__, state.structure_str, self.direction)
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(state=%s, x=%s, y=%s, text=%s)" % \
                   (self.__class__.__name__, state.structure_str, self.x, self.y, self.text)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
            raise InvalidEventException(msg)
